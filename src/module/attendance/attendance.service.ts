...
export default class AttendanceService {
    ...
    async getReportsSummary(query: any) {
        ...
        if (mode === "monthly") {
            const monthlyRows = await this.buildMonthlyReportRows(query, fromDate, toDate, timezone);
            const totalEmployeesMatch = this.buildEmployeeMatchQuery(query);
            const totalEmployees = await EmployeeModel.countDocuments(totalEmployeesMatch);
            const present = monthlyRows.pagination.total;
            const monthlyTotals = monthlyRows.totals ?? { totalDurationMinutes: 0, lateCount: 0, missingExits: 0, overtimeDays: 0 };
            const totalDurationMs = monthlyTotals.totalDurationMinutes * 60000;
            const avgMinutes = present > 0 ? Math.round(totalDurationMs / present / 60000) : 0;

            return {
                present,
                absent: Math.max(0, totalEmployees - present),
                late: monthlyTotals.lateCount,
                avgHours: `${Math.floor(avgMinutes / 60)}h ${avgMinutes % 60}m`,
                missingExit: monthlyTotals.missingExits,
                overtime: monthlyTotals.overtimeDays,
                totalEmployees,
            };
        }
    ...
    getReportsRows = async (query: any) => {
        ...
        if (mode === "monthly") {
            ...
            return {
                ...
                pagination: monthlyRows.pagination,
            };
        }
    ...
    private formatMinutesToHoursMinutes(totalMinutes: number) {
    ...
    private matchesTextFilter(value: string | null | undefined, filter: string | undefined) {
    ...
    // Helper: Check if logs contain any EXIT-type event
    private hasExitEvent(logs: any[]): boolean {
        ...
        return logs.some((log) => exitEventTypes.includes(log.eventType));
    }

    // Helper: Check if first event is late (after office start time)
    private isFirstEventLate(firstEventMs: number, timezone: string): boolean {
    ...
    private async fetchMonthlyPresenceLogs( employeeIds: string[], monthStartDate: string,
        ...
        const { dayStartMs } = this.getDayBounds(monthStartDate, timezone);
        const { dayEndMs } = this.getDayBounds(monthEndDate, timezone);
        ...
        const logs = await PresenceLogModel.find({
            ...
            .sort({ employeeId: 1, occurredAt: 1 })
            .lean();

        return logs;
    }

    // Optimized: Deduplicate logs in Node.js (same camera + time < 2 min)
    private deduplicateMovementLogs(logs: any[]): any[] {
        ...
        for (const log of logs) {
            ...
            if (String(log.employeeId) !== String(prevLog.employeeId)) {
                deduped.push(log);
                prevLog = log;
                continue;
            }

            // Check if same camera AND within 2 minutes
            const timeDiffMs = Math.abs(Number(log.occurredAt) - Number(prevLog.occurredAt));
            if (log.cameraCode === prevLog.cameraCode && timeDiffMs < 2 * 60 * 1000) {
                // Skip duplicate
                logger.debug(
                    `Skipping duplicate event for ${log.employeeId} at camera ${log.cameraCode}, diff: ${timeDiffMs}ms`
                );
                continue;
            }
    ...
    // Optimized: Group deduped logs by employee and date
    private groupMovementLogsByEmployeeAndDate(logs: any[], timezone: string) {
        const groupedData = new Map<string, Map<string, any[]>>();

        for (const log of logs) {
            const empId = String(log.employeeId);
            const dateStr = this.toAttendenceDate(Number(log.occurredAt), timezone);

            if (!groupedData.has(empId)) {
                groupedData.set(empId, new Map());
            }

            const employeeData = groupedData.get(empId)!;
            if (!employeeData.has(dateStr)) {
                employeeData.set(dateStr, []);
            }

            employeeData.get(dateStr)!.push(log);
        }

        return groupedData;
    }

    // Optimized: Build timeline rows for monthly export (single DB query)
    private async buildMonthlyTimelineRowsOptimized(
        ...
        // SINGLE DB QUERY: Fetch all logs for month
        logger.info(
            `Fetching monthly presence logs for ${timelineEmployeeIds.length} employees from ${monthStartDate} to ${monthEndDate}`
        );
        const allLogs = await this.fetchMonthlyPresenceLogs(
            timelineEmployeeIds,
            monthStartDate,
            monthEndDate,
            timezone
        );
        logger.info(`Fetched ${allLogs.length} raw logs from DB`);

        // Deduplicate in Node.js
        const dedupedLogs = this.deduplicateMovementLogs(allLogs);
        logger.info(`After deduplication: ${dedupedLogs.length} logs`);
        ...
        // Build rows: iterate employees, then dates per employee
        for (const empId of timelineEmployeeIds) {
            ...
            // Sort dates for consistent order
            ...
            for (const dateStr of sortedDates) {
                ...
                const firstSeenAt = formatTime(Number(firstLog.occurredAt)); // No seconds for summary
                const lastSeenAt = formatTime(Number(lastLog.occurredAt)); // No seconds for summary
                const totalEvents = dayLogs.length;

                // Calculate work hours: duration from first event to last event
                ...
                // Determine late status: if first event is after office start time
                const isLate = this.isFirstEventLate(firstEventMs, timezone);
                const lateStatus = isLate ? "Yes" : "No";

                // Determine missing exit: if no EXIT-type event found
                ...
                for (const log of dayLogs) {
                    ...
                    // Determine if event is EXIT or ENTRY type
                    const exitEventTypes = [
                        "EXIT_DETECTED",
                        "EXIT_PENDING",
                        "PENDING_EXIT",
    ...
    private buildEmployeeMatchQuery(query: any) {
        ...
        if (query.employeeId) {
            andConditions.push({
                $or: [
                    { id: query.employeeId },
                    ...(ObjectId.isValid(query.employeeId) ? [{ _id: new ObjectId(query.employeeId) }] : []),
                ],
            });
        }

        if (query.employeeName) {
            andConditions.push({ name: { $regex: query.employeeName.trim(), $options: "i" } });
        }

        if (query.department) {
            andConditions.push({ department: { $regex: query.department.trim(), $options: "i" } });
        }
    ...
    private async buildMonthlyReportRows(query: any, fromDate: string, toDate: string, timezone: string) {
        const attendanceMatch: Record<string, any> = {
            date: { $gte: fromDate, $lte: toDate },
        };
        ...
        const workingDays = this.countWorkingDays(fromDate, toDate, timezone);
        const officeDurationMs = Math.max(0, envConfig.officeEndTime - envConfig.officeStartTime);
        const statusFilter = Array.isArray(query.status) ? query.status : [];

        const rows = Array.from(groupedByEmployee.entries()).map(([employeeKey, employeeSessions]) => {
            const orderedSessions = [...employeeSessions].sort((left, right) => Number(left.entryAt) - Number(right.entryAt));
            ...
            for (const [, day] of groupedByDay) {
                ...
                if (day.firstEntryAt && this.isLateEntry(Number(day.firstEntryAt), timezone)) {
                    lateCount += 1;
                }

                if (day.sessions.some((session: any) => !session.exitAt)) {
                    missingExits += 1;
                }

                if (Number(day.totalDurationMs ?? 0) > officeDurationMs) {
                    overtimeDays += 1;
                }
            }
            ...
            const avgWorkHours = presentDays > 0
                ? this.formatMinutesToHoursMinutes(totalDurationMs / 60000 / presentDays)
                : "0h 0m";

            return {
                id: `monthly_${toDate}_${employeeKey}`,
                ...
                overtimeDays,
                missingExits,
                _totalDurationMinutes: Math.round(totalDurationMs / 60000),
            };
        }).filter(Boolean) as Array<Record<string, any> & { _totalDurationMinutes: number }>;
        ...
        rows.sort((left, right) => {
            ...
            if (sortBy === "presentDays") {
                comparison = left.presentDays - right.presentDays;
            } else if (sortBy === "lateCount") {
                comparison = left.lateCount - right.lateCount;
            } else if (sortBy === "avgWorkHours") {
                comparison = Number(left._totalDurationMinutes) - Number(right._totalDurationMinutes);
            } else if (sortBy === "overtimeDays") {
                comparison = left.overtimeDays - right.overtimeDays;
            } else if (sortBy === "missingExits") {
                comparison = left.missingExits - right.missingExits;
            } else if (sortBy === "department") {
                comparison = String(left.department ?? "").localeCompare(String(right.department ?? ""));
            } else {
        ...
        const total = rows.length;
        const totalPages = Math.ceil(total / pageSize);
        const paginated = rows.slice((page - 1) * pageSize, (page - 1) * pageSize + pageSize).map(({ _totalDurationMinutes, ...rest }) => rest);

        return {
            rows: paginated,
            pagination: { page, pageSize, total, totalPages },
            totals: {
                totalDurationMinutes: rows.reduce((sum, row) => sum + Number(row._totalDurationMinutes ?? 0), 0),
                lateCount: rows.reduce((sum, row) => sum + Number(row.lateCount ?? 0), 0),
                missingExits: rows.reduce((sum, row) => sum + Number(row.missingExits ?? 0), 0),
                overtimeDays: rows.reduce((sum, row) => sum + Number(row.overtimeDays ?? 0), 0),
            },
        };
    }
    ...
    exportAttendanceReport = async (query: ReportsExportBodyDTO) => {
        ...
        if (timelineEmployeeIds.length === 0) {
            ...
            if (mode === "monthly") {
                // Optimized monthly mode: SINGLE DB query for all month's logs
                const month = rowsQuery.month ?? DateTime.now().setZone(timezone).toFormat("yyyy-MM");
                const { monthStartDate, monthEndDate } = this.getMonthBounds(month, timezone);

                const monthTimelineRows = await this.buildMonthlyTimelineRowsOptimized(
                    timelineEmployeeIds,
                    monthStartDate,
                    monthEndDate,
                    timezone
                ...
                timelineRows.push(...monthTimelineRows);
            } else {
